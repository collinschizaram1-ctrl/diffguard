...
export default function Sidebar({ activeView, setActiveView, rpcStatus, sidebarOpen, setSidebarOpen }: SidebarProps) {
  ...
  return (
    ...
    >
      <div className="w-72 flex-1 flex flex-col h-full justify-between shrink-0">
        {/* Sidebar Top Area */}
        ...
        <div className="flex-1 overflow-y-auto px-3.5 py-4 space-y-5">
          ...
          {/* Top Quick Action Tabs (Security, Build, Image) */}
          <div className="grid grid-cols-3 gap-1.5 bg-[#0D0F14]/60 p-1.5 rounded-xl border border-[#222631]/40">
            {/* Security tab */}
            <button
            ...
            >
              <Shield className={`w-4 h-4 mb-1 ${activeView === "audit" ? "text-[#0088FF]" : ""}`} />
            ...
            >
              <Hammer className={`w-4 h-4 mb-1 ${activeView === "sandbox" ? "text-[#0088FF]" : ""}`} />
            ...
            >
              <ImageIcon className={`w-4 h-4 mb-1 ${activeView === "recon" ? "text-[#0088FF]" : ""}`} />
              <span className="text-[10px] font-medium">Recon</span>
            </button>
          </div>

          {/* Simple Navigation shortcuts list */}
          <div className="space-y-1">
            <button 
              onClick={() => setActiveView("audit")}
              className="w-full text-left px-3 py-2 rounded-lg text-xs font-medium hover:bg-[#1E222D] text-gray-300 hover:text-white transition-all flex items-center gap-2.5 cursor-pointer"
            >
            ...
            <button 
              ...
              className="w-full text-left px-3 py-2 rounded-lg text-xs font-medium hover:bg-[#1E222D] text-gray-300 hover:text-white transition-all flex items-center gap-2.5 cursor-pointer"
            >
              <ToyBrick className="w-3.5 h-3.5 text-gray-500" />
              Plugins
            </button>
          ...
          <div className="space-y-1.5">
            ...
            >
              ...
              {operationsOpen ? <ChevronDown className="w-3 h-3" /> : <ChevronUp className="w-3 h-3" />}
            </button>
          ...
          <div className="space-y-1.5">
            ...
            >
              <span>Projects</span>
              {projectsOpen ? <ChevronDown className="w-3 h-3" /> : <ChevronUp className="w-3 h-3" />}
            </button>
      ...
      {/* Connection Indicator Footer instead of mock user account profile */}
      <div className="p-4 border-t border-[#222631]/60 bg-[#101217] shrink-0">
